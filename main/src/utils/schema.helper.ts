...
// SchemaModel дээр тохируулсан нэмэлт тохиргоогоор энгийн филтер, лист, мэдээлэл татах ерөнхий дундын дунамик функцууд
module.exports = {
  getShort(schema) {
    const names: string[] = []
  ...
  getSchema(name, fields: any[] = [], isReport = false) {
    let data: any[] = []
    if (temp.hasOwnProperty(name)) {
      data = temp[name]
    } else {
      console.log('--getSchema--name-----', name)
      console.log('--getSchema--mongoose.models-----', mongoose.models)
      const result = mongoose.models(name).schema
      console.log('--getSchema--result-----', result)
      const model : any[] = []
      Object.keys(result.paths).forEach((fieldKey) => {
        const fieldObj = result.paths[fieldKey]
    ...
    if (fields && fields.length > 0) {
      if (isReport) {
        ...
        fields.forEach((f: any) => {
          if (f.indexOf('.') > -1) {
            const found = data.find(a => _.isEqual(a.path, f.split('.')))
            if (found) {
              returnData.push(found)
            } else {
              const splitted = f.split('.')
              splitted.pop()
              const found = data.find(a => _.isEqual(a.path, splitted))
              if (found && found.type === 'ObjectID') {
                returnData.push(found)
              }
            }
          } else {
            const found = data.find(a => a.key === f)
            if (found) {
              returnData.push(found)
            }
          }
      ...
      return data.filter(a => fields.indexOf(a.path[0]) > -1)
    }
    return data.filter(a => a.allowed === true)
  },
  getShortFields(collection, isReport = false) {
    const collectionSchema = this.getSchema(collection)
    const fields = { _id: 1 }
    collectionSchema.filter((c) => isReport ? c.report : c.short).forEach((collectionField) => {
  ...
  getSchemaJoiErrors(modelName, errors) {
    const msgs: any[] = []
    const fields = this.getSchema(modelName)
    errors.forEach((msg) => {
  ...
  getLookupColumns(collection, fields, isColumn = false, modelSchema = null) {
    const lookup: any[] = []
    const schema = modelSchema !== null ? modelSchema : this.getSchema(collection, fields = [])
    schema.forEach((field) => {
  ...
  findAggregation(collection, fields, aggregation, project, exclude: string[] = []) {
    const schema = this.getSchema(collection, fields)
    schema.forEach((field) => {
        ...
        if (Array.isArray(data[object.path[0]])) {
          ...
          data[object.path[0]].forEach(nested => {
            if (object.type === 'ObjectID' && nested[object.path[1]]) {
              ids.push(mongoose.Types.ObjectId(nested[object.path[1]]))
          ...
          if (Object.keys(data[object.path[0]]).length > 0) {
            promises.push(this.findOne(object.collection, object.lookupProject ? object.lookupProject : [], { _id: { $eq: mongoose.Types.ObjectId(data[object.path[0]]) } }))
          }
        }
      }
    })
  ...
  async listFilter(model, query, filter = {}, returnFilter = false, additionalAggregation = [], additionalAllFilter = [], modelSchema = null, inlineFilter = {}) {
    const schema = modelSchema !== null ? modelSchema : this.getSchema(model)
            ...
            if (fieldConfig.type === 'String') {
              ...
              if (Array.isArray(fieldValue)) {
                ...
                fieldValue.forEach((fieldId) => {
                  ...
                  list.push(mongoose.Types.ObjectId(id))
                })
                ...
                if (fieldValue.hasOwnProperty('_id') || ObjectId.isValid(fieldValue)) {
                  ...
                  if (fieldValue !== '') {
                    filterConfig = { $eq: mongoose.Types.ObjectId(fieldValue) }
                  }
  ...
  async findAll(collection, fields, filter, sort = null) {
    const aggregation = this.findAggregation(collection, fields, [{ $match: filter}], {})
    if (sort) {
      aggregation.push({ $sort: sort })
    }
    const list = await mongoose.model(collection).aggregate(aggregation)
    const promises: any[] = []
    list.forEach((a) => {
      promises.push(this.lookup(a, this.getLookupColumns(collection, fields, true)))
    })
  ...
  async findOne(collection, fields, filter, exclude = []) {
    ...
    const aggregateResponse = await mongoose.model(collection).aggregate(aggregation)
